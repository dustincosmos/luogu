...
int main()
{
    ...
    for(int i=0;i<num;i++)
    {
        cin>>stu[i].name>>stu[i].ex>>stu[i].cx>>stu[i].ganbu>>stu[i].xibu>>stu[i].essay;
    }
    for(int i=0;i<num;i++)
        stu[i].sum=0;
    int max=stu[0].sum;
    ...
    for(int i=0;i<num;i++)
    {
        if(stu[i].ex>85&&stu[i].cx>80)
            stu[i].sum+=4000;
        if(stu[i].ex>90)
            stu[i].sum+=2000;
        if(stu[i].ex>85&&stu[i].xibu=='Y')
            stu[i].sum+=1000;
        if(stu[i].cx>80&&stu[i].ganbu=='Y')
            stu[i].sum+=850;
        if(stu[i].ex>80&&stu[i].essay>=1)
            stu[i].sum+=8000;
        allsum+=stu[i].sum;
        if(stu[i].sum>max)
        {
    ...
    cout<<stu[index].name<<endl<<stu[index].sum<<endl<<allsum;
    system("pause");
}